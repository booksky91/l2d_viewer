# 🚨 !WARNING
본 저장소는 사전 빌드된 바이너리 실행 파일(Release 빌드)을 직접 제공하지 않습니다.  
사용 목적에 맞게 아래의 빌드 가이드를 참고하여 환경을 구성한 후, 직접 빌드하여 사용하여야 합니다.
...
- **GIF 디더링 및 색상 최적화**: GIF 포맷 내보내기 시 디더링 모드(None, Bayer, Floyd-Steinberg, Sierra) 및 최대 색상 제한(64~256 색상 범위)을 세부 조절하여 부드러운 그러데이션과 최적의 용량 배출을 보장합니다.
...
### 7. 모델별 고프레임 강제 모션 보간 (Stepped Interpolation)
- **프레임 Stuttering 방지**: 모션 데이터 파일(`.motion3.json`) 내에 계단식(Stepped)으로 키프레임 간 바인딩이 되어 있을 경우, 60fps / 120fps+ 등의 고프레임 내보내기 시 프레임이 중복되어 뚝뚝 끊기는(Stuttering) 현상이 발생합니다. 이를 개선하기 위해 강제 보간 기능을 탑재했습니다.
- **다양한 보간 모드**: `Render` 탭에서 각 모델마다 개별적으로 **Off (Stepped)**, **Linear (선형)**, **Smoothstep (Hermite 곡선 $3t^2 - 2t^3$)** 옵션을 실시간 제어할 수 있어 30fps/15fps 키프레임 사이를 프레임 레이트 제한 없이(60fps, 120fps, 144fps 등) 실시간 수학적으로 매끄럽게 연결합니다. 이 모드 역시 프로젝트 저장 및 백업 시에 완벽하게 연동됩니다.
